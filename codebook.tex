% =============================================================
% Codebook 主檔說明
% - 目標：在最少頁數內排入常用演算法 / 資料結構程式碼。
% - 版面：使用 twocolumn 壓縮，手動設定窄邊界，字體縮小。
% - 編譯：建議使用 xelatex (需支援 fontspec / xeCJK)。
% - 可調整項目：行號 (listings)、字型、是否加入更多章節。
% =============================================================
\documentclass[a4paper,10pt,twocolumn,oneside]{article}
\setlength{\columnsep}{10pt}                                                              %兩欄模式的間距
\setlength{\columnseprule}{0pt}                                                                %兩欄模式間格線粗細

\usepackage{amsthm}								%定義，例題
\usepackage{amssymb}
%\usepackage[margin=2cm]{geometry}
\usepackage{fontspec}								%設定字體
\usepackage{color}
\usepackage[x11names]{xcolor}
\usepackage{listings}								%顯示code用的
%\usepackage[Glenn]{fncychap}						%排版，頁面模板
\usepackage{fancyhdr}								%設定頁首頁尾
\usepackage{graphicx}								%Graphic
\usepackage{enumerate}
\usepackage{titlesec}
\usepackage{amsmath}
\usepackage{tikz}
\usepackage{xpatch}
\usepackage[CheckSingle, CJKmath]{xeCJK}
% \usepackage{CJKulem}
%\usepackage[T1]{fontenc}
\titlespacing{\section}{0cm}{0cm}{0cm}
\titlespacing{\subsection}{0cm}{0cm}{0cm}
\usepackage{amsmath, courier, listings, fancyhdr, graphicx}
\topmargin=0pt
\headsep=5pt
\textheight=780pt
\footskip=0pt
\voffset=-40pt
\textwidth=545pt
\marginparsep=0pt
\marginparwidth=0pt
\marginparpush=0pt
\oddsidemargin=0pt
\evensidemargin=0pt
\hoffset=-42pt
\makeatletter
\renewcommand{\numberline}[1]{#1\hspace{0.8em}} % 增加編號與標題間的固定空白
\makeatother
%\usepackage{draftwatermark}
%\SetWatermarkText{Jackis666}   % 浮水印文字
%\SetWatermarkScale{0.5}           % 大小（0.5~2之間調）
%\SetWatermarkColor[gray]{0.9}     % 顏色（越接近1越淡）*/

%\renewcommand\listfigurename{圖目錄}
%\renewcommand\listtablename{表目錄} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\setmainfont [				%主要字型
    Path = .fonts/ttf/,
    UprightFont = *-Regular,
    BoldFont = *-Bold,
    ItalicFont = *-Italic
  ] {Consolas}

\setmonofont [        
    Path = .fonts/ttf/,
    UprightFont = *-Regular
  ] {Monaco}

\setCJKmainfont[
  Path = .fonts/ttf/,        % 你放字型檔的資料夾（相對於主 .tex 檔案）
  Extension = .ttf,
  UprightFont = *-Regular,
  BoldFont = *-Bold,
]{NotoSansTC}

\XeTeXlinebreaklocale "zh"						%中文自動換行
\XeTeXlinebreakskip = 0pt plus 1pt				%設定段落之間的距離
\setcounter{secnumdepth}{3}						%目錄顯示第三層

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\makeatletter
\lst@CCPutMacro\lst@ProcessOther {"2D}{\lst@ttfamily{-{}}{-{}}}
\@empty\z@\@empty
\makeatother
\lstset{											% Code顯示
language=C++,										% the language of the code
basicstyle=\footnotesize\ttfamily, 						% the size of the fonts that are used for the code
%numbers=left,										% where to put the line-numbers
numberstyle=\footnotesize,						% the size of the fonts that are used for the line-numbers
stepnumber=1,										% the step between two line-numbers. If it's 1, each line  will be numbered
numbersep=5pt,										% how far the line-numbers are from the code
backgroundcolor=\color{white},					% choose the background color. You must add \usepackage{color}
showspaces=false,									% show spaces adding particular underscores
showstringspaces=false,							% underline spaces within strings
showtabs=false,									% show tabs within strings adding particular underscores
frame=false,											% adds a frame around the code
tabsize=2,											% sets default tabsize to 2 spaces
captionpos=b,										% sets the caption-position to bottom
breaklines=true,									% sets automatic line breaking
breakatwhitespace=false,							% sets if automatic breaks should only happen at whitespace
escapeinside={\%*}{*)},							% if you want to add a comment within your code
morekeywords={*},									% if you want to add more keywords to the set
keywordstyle=\bfseries\color{Blue1},
commentstyle=\itshape\color{Red4},
stringstyle=\itshape\color{Green4},
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}
\pagestyle{fancy}
\fancyfoot{}
%\fancyfoot[R]{\includegraphics[width=20pt]{ironwood.jpg}}
\fancyhead[L]{No\_AC\_No\_Home}
\fancyhead[R]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\contentsname}{Contents} 

\newcommand{\includetex}[2]{
  \subsection{#1}
  \input{#2}
  \vspace{-1.2em}
}
\scriptsize
\tableofcontents


% \newpage
%sectionstart  % ===== 以下依分類匯入各程式碼檔案 =====

\section{Basic}
    \subsection{default}
    \lstinputlisting{basic/default.cpp}

    \subsection{奇妙的東西}
    \lstinputlisting{basic/coolthings.cpp}

    \subsection{random}
    \lstinputlisting{basic/random.cpp}

%====================================================


%====================================================

\section{Binary Search}
    \subsection{二分搜}
    \lstinputlisting{binary search/binary search.cpp}

    \subsection{三分搜}
    \lstinputlisting{binary search/tenary search.cpp}

%====================================================

\section{DataStructure}
    \subsection{DSU(並查集)}
    \lstinputlisting{datastructure/dsu.cpp}
    
    \subsection{FenwickTree/BIT}
    \lstinputlisting{datastructure/BIT.cpp}

    \subsection{二維BIT}
    \lstinputlisting{datastructure/BIT_2D.cpp}

    \subsection{Seg/線段樹}
    \lstinputlisting{datastructure/Seg.cpp}

    \subsection{李超線段樹}
    \lstinputlisting{datastructure/lichaoseg.cpp}

    \subsection{SegTree Beats/線段樹 Beats}
    \lstinputlisting{datastructure/Segment_tree_beats.cpp}

%====================================================

\section{Graph}

    \subsection{Topo/拓樸}
    \lstinputlisting{graph/topo.cpp}

    \subsection{bellman/邊長鬆弛}
    \lstinputlisting{graph/bellman.cpp}

    \subsection{ola/歐拉}
    \lstinputlisting{graph/ola.cpp}

    \subsection{LCA/最近公共祖先}
    \lstinputlisting{graph/lca.cpp}


    \subsection{Scc/強連通分量}
    \lstinputlisting{graph/Scc.cpp}

    \subsection{Bcc/雙連通分量}
    \lstinputlisting{graph/Bcc.cpp}

    \subsection{最大團}
    \lstinputlisting{graph/maxcilque.cpp}

    \subsection{平均最小團}
    \lstinputlisting{graph/平均最小團.cpp}

    \subsection{支配點}
    \lstinputlisting{graph/支配點.cpp}

%====================================================

\section{dynamic programming}
    \subsection{SOS DP}
    \lstinputlisting{dynamic programming/SOS.cpp}

    \subsection{p-median}
    \lstinputlisting{dynamic programming/p_median.cpp}

%====================================================

\section{Computational Geometry}
    \subsection{defult}
    \lstinputlisting{Computational Geometry/defult.cpp}

    \subsection{Circle}
    \lstinputlisting{Computational Geometry/circle.cpp}

    \subsection{極座標排序}
    \lstinputlisting{math/sort_of_point.cpp}

    \subsection{ConvexHull/凸包}
    \lstinputlisting{Computational Geometry/ConvexHull.cpp}

    \subsection{PtSegDist}
    \lstinputlisting{Computational Geometry/PtSegDist.cpp}

    \subsection{兩線相交}
    \lstinputlisting{Computational Geometry/IntersectionOfLines.cpp}

    \subsection{點是否在多邊形內}
    \lstinputlisting{Computational Geometry/inpoly.cpp}
    
    \subsection{最遠點對/選轉卡尺}
    \lstinputlisting{Computational Geometry/選轉卡尺.cpp}

    \subsection{最近點對}
    \lstinputlisting{Computational Geometry/closet pair.cpp}


%===================================================

\section{Flow}

    \subsection{km/二分圖最大權匹配}
    \lstinputlisting{flow/km.cpp}

    \subsection{Dinic}
    \lstinputlisting{flow/dinic.cpp}

    \subsection{hungarian/匈牙利演算法}
    \lstinputlisting{flow/hungarian.cpp}

    \subsection{SPFA最小費流}
    \lstinputlisting{flow/最小花費最大流SPFA.cpp}

    \subsection{dijk\_flow/最小費流}
    \lstinputlisting{flow/dijk_flow.cpp}

    \includetex{差分約束系統}{graph/conclusion.tex}
    \vspace{1.2em}

%====================================================

\section{String}
    \subsection{KMP}
    \lstinputlisting{string/KMP.cpp}

    \subsection{Z-value}
    \lstinputlisting{string/zvalue.cpp}

    \subsection{馬拉車}
    \lstinputlisting{string/馬拉車.cpp}

    \subsection{minRotation/最小字典序旋轉}
    \lstinputlisting{string/minRotation.cpp}

    \subsection{SAIS}
    \lstinputlisting{string/SAIS.cpp}

    \subsection{Trie/字典樹}
    \lstinputlisting{string/trie.cpp}

    \subsection{ACautomata/AC自動機}
    \lstinputlisting{string/ACautomata.cpp}

    


%====================================================

\section{酷東西可能會用到}
    \subsection{areaofrect/矩形聯集面積}
    \lstinputlisting{useful/areaofrect.cpp}

%====================================================
\section{Math}
    \subsection{basic}
    \lstinputlisting{math/basic.cpp}

    \subsection{快速幂}
    \lstinputlisting{math/pow.cpp}

    \subsection{大數運算}
    \lstinputlisting{math/bigmath.cpp}

    \subsection{Prime}
    \lstinputlisting{math/Prime.cpp}

    \subsection{ExGCD}
    \lstinputlisting{math/exgcd.cpp}

    \subsection{Phi}
    \lstinputlisting{math/phi.cpp}

    \subsection{Euler/歐拉函數}
    \lstinputlisting{math/euler.cpp}

    \subsection{miller rabin/判斷質數}
    \lstinputlisting{math/miller_rabin.cpp}

    \subsection{Pollard Rho/質因數分解}
    \lstinputlisting{math/pollard_rho.cpp}

    \subsection{FFT/傅立葉轉換(多項式相乘)}
    \lstinputlisting{math/FFT.cpp}

    \subsection{josephus/約瑟夫問題}
    \lstinputlisting{math/josephus.cpp}

    \subsection{指數塔}
    \lstinputlisting{math/指數塔.cpp}

    \subsection{BSGS}
    \lstinputlisting{math/BSGS.cpp}

    \subsection{$x^2 \pmod p == a$}
    \lstinputlisting{math/DiscreteSqrt.cpp}

    \includetex{Theorem}{math/theorem.tex}
    \vspace{1.2em}

    %\includetex{Prime}{math/prime.cpp}
    %\vspace{1.2em}
%====================================================
    \includegraphics[scale=0.2]{photo/mygo_frame_haruhikage.jpeg}


%====================================================
\newcommand{\halfgridpage}{
    \begin{tikzpicture}[every node/.style={minimum size=1cm-\pgflinewidth, outer sep=10pt}, scale=2]
        \draw[step=0.2cm,color=gray] (0,0) grid (4,13);
        \draw[step=1cm,color=black,line width=0.4mm] (0,0) grid (4,13);
    \end{tikzpicture}
    \newpage
}
\newcommand{\gridpage}{
    \centering
\begin{tikzpicture}[every node/.style={minimum size=1cm-\pgflinewidth, outer sep=10pt}, scale=2]
    \draw[step=0.2cm,color=gray] (0,0) grid (9,12);
    \draw[step=1cm,color=black,line width=0.4mm] (0,0) grid (9,12);
\end{tikzpicture}
    \newpage
}
\onecolumn
\gridpage
\gridpage



\end{document}